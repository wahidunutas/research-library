<div class="tab-content" id="custom-tabs-one-tabContent">
    <div class="tab-pane fade show active" id="home-pending" role="tabpanel" aria-labelledby="pending">
        <ul class="nav nav-tabs" id="tabs-isi" role="tablist">
            <li class="nav-item">
                <a class="nav-link active" id="custom-tabs-four-profile-tab" data-toggle="pill" href="#magang-tabs" role="tab" aria-controls="magang-tabs" aria-selected="false">Karya Ilmiah</a>
            </li>
            <li class="nav-item">
            ...
            <div class="tab-pane fade" id="skripsi-tabs" role="tabpanel" aria-labelledby="custom-tabs-four-profile-tab">
                <table class="table table-hover">
                    ...
                    if (isset($_POST['cari'])) {
                        $keyword = $_POST['keyword'];
                        $no = $limitStart + 1;
                        $query = $koneksi->query("SELECT * FROM jurnal JOIN tipe_jurnal ON jurnal.tipe_jurnal=tipe_jurnal.id_tipe_jurnal WHERE jurnal.id_author = '$id' AND judul LIKE '%$keyword%' OR status LIKE '%$keyword%'");
                    } else {
                        $query = $koneksi->query("SELECT * FROM jurnal JOIN tipe_jurnal ON jurnal.tipe_jurnal=tipe_jurnal.id_tipe_jurnal WHERE jurnal.id_author = '$id' ORDER BY jurnal.id_jurnal DESC LIMIT $limitStart, $limit");
                    }
                    // $nomer = 1;
                    // while ($data = $query->fetch_assoc()) {
                    //     $judul = substr($data['judul'], 0, 50) . '[...]';
