...
<body class="hold-transition sidebar-mini">
	...
	$ceks = mysqli_num_rows($sql);
	// $sqla = $koneksi->query("SELECT * FROM author WHERE email='$email'");
	// $ceka = mysqli_num_rows($sqla);
	...
	if ($pw !== $pw2) {
		...
		echo "<meta http-equiv='refresh' content='1;url=../../../?p=addadmin'>";
	} elseif ($ceks == 1) {
		echo "<script>
                Swal.fire({
                icon: 'error',
		...
		$koneksi->query("INSERT INTO admin(nama, jabatan, no_telepon, email)VALUES('$nama', '$jabatan', '$no', '$email')");

		$id = $koneksi->insert_id;
		$koneksi->query("INSERT INTO akses(id_admin, nip, password, role, verif_code, is_verif)VALUES('$id', '$nip', '$pw', '$status', '$code', '1')");
		...
		if (!$mail->send()) {
			...
			echo "<meta http-equiv='refresh' content='3;url=../../../index.php?p=addadmin'>";
			//Section 2: IMAP
